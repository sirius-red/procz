...
- `children()` now returns a PID list sorted in ascending order for stable
  results across platforms, making `killTree()` traversal deterministic.

## [3.4.0] - 2026-01-19

### Changed

- `resourceUsage()` now also provides `start_time_unix_ns` (unix epoch
  nanoseconds) when available, while keeping platform-specific
  `start_time_ns`.
...
- `userNormalized(allocator, pid)` helper that returns a normalized user
  identity string (`uid:<n>` or `sid:<value>`).

## [3.2.0] - 2026-01-19

### Added

- `exeName(allocator, pid)` helper that returns a canonical executable base
  name derived from `exePath()`.
